 ...
 * behavior constructs invariant-bearing data whose invariant fails (spec 9.4). It mirrors
 * {@code 復号失敗} on the decoder side. Every behavior that constructs invariant-bearing data
 * must include this arm in its output sum so a violation has somewhere to go (spec 22.3).
 */
