...
| [0052](0052-recursion-total-by-default.md) | Recursion is total by default, checked by structural recursion (a recursive call passes a strictly smaller part of a parameter); `partial` opts out; numeric/index recursion must be `partial` (no inductive `Nat`); stdlib `foldFrom` is trusted total; example evaluation is time-bounded | `[#fn-declaration]`, `[#examples]` |
