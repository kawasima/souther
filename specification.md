...
| `// 依存: 製品カタログ` / `// 副作用: メール送信` | 実装（`fn`）を書かない `behavior`（Java実装を注入） |
...
- `T` の Decoder
- `constructs T` を宣言した behavior の実装（Souther の `fn`、または注入された Java 実装）
- コンパイラ生成コード

戻り値型に `T` を書いただけの behavior は `T` を生成できない。これは、仕様DSLで「検証済みメールアドレスは検証済みであることが型で保証される」（parse-don't-validate）と述べた不変条件を、実装で破れないようにするための規則である。

権限を持つのは behavior であって、その実装ではない。だから `constructs` は behavior に書き（12.3）、二つの実装経路——Souther の `fn` と Java 実装——が同じ宣言から権限を得る。どの behavior にも対応しない助けの `fn`（13.1）は data を構築できない。
...
behavior が言うのは**仕様だけ**で、実装は持たない。実装は `fn` として書くか（13章）、書かなければ Java から注入される（13.2）。仕様DSLに載る振る舞いだけが behavior であり、実装の助けは `fn` に留まる。そうしておくと、behavior の一覧が仕様DSLの一覧と食い違わない。
...
```text
behavior findMember = (id: MemberId) -> Member | FindMemberError    // fn が無い
```

実装はJava側から注入する。実装（`fn`）を書かないことが、外から与えるという意味である（13.2）。依存（外部を読むだけ）と副作用（外部を変える）の区別は用途を示すドキュメントであり、値の合成規則には影響しない。
...
`->` は型の矢印で、behavior の宣言（12章）と `fn` の引数に書く関数型（13.1）に出る。`=>` は無名のもの——`match` のアーム（16.3）とブロック（12.5）——に使う。名前付きの定義は `=` で本体を導く（`fn f (x) = ...`）。SML が `fun f x = e` と `fn x => e` を分けているのと同じ分業である。
...
`Date` / `DateTime` は仕様DSLの `年月日` / `年月日時分秒` に対応する。値の生成は Decoder（ISO8601文字列から）を経る。現在時刻の取得は言語内では行わず、実装を書かない behavior として宣言する（13.2）。
...
```text
// 実装（fn）で単位を構築する。behavior の constructs を要する（2.1・12.3）
...
許可: 比較、論理演算、算術演算、フィールド参照、組み込み関数、ブロック、そして**要求集合が空の `fn`**。

```text
fn 正の数 (v: Int) = v >= 0

data 金額 = { value: Int  invariant 正の数(value) }
data カゴ = { items: List<明細>  invariant all(items, x => 正の数(x.数量)) }
```

呼べるのは behavior ではなく `fn` である（13章）。invariant は data の生成経路で走るので、そこで外界に触れられては困る。behavior は外界に触れうる——実装を書かなければ Java 実装が注入される——ので、invariant からは呼べない。業務規則に名前を付けたいときは `fn` にする。

条件は宣言せず、推論する。`fn` が invariant から呼べるのは次のすべてを満たすときで、いずれも静的に判定できる。

- 推移的な要求集合が空である（実装を書かない behavior を一度も呼ばない。17章）
- data を生成しない
- 再帰しない

「要求集合が空」は 17章の意味の純粋性より強い条件である。`fn 事前承認する` は外界に直接触れないので 17章の意味では純粋だが、`現在時刻()` を呼ぶので要求集合は空でなく、invariant からは呼べない。

禁止: Decoder、Encoder、behavior の呼び出し、dataの生成、再帰、非決定的処理。
...
behavior は内部dataに対する振る舞いの**仕様**を定義する。所属を持たないトップレベルの入出力関係である。**本体を持たない。**

宣言は `behavior 名前 = 右辺` の形をとる。右辺は引数リストと出力（12.1）か、`>>` による合成（14章）である。名前をその入出力関係に束縛する、と読める。仕様DSLの `behavior` と同じ形である。

実装は別に書く。`fn` として Souther で書くか（13章）、書かなければ Java から注入される（13.2）。仕様DSLが `// 依存:` とコメントで注記したものは、**実装を書かない behavior** になる。
...
// 仕様DSL: // 依存: 時刻取得
behavior 現在時刻 = () -> DateTime          // fn が無い → Java から注入
```

仕様と実装を分けるのは、`data` が既にそうしているからではなく、**仕様DSLの1行がそのまま残るようにするため**である（28章）。`data` に `invariant` が乗るのと同じく、`behavior` には `constructs` が乗る。どちらも、仕様DSLがコメントに預けたものを Souther が実行可能にした分である（1.1）。
...
パラメータ名は読み手のためのもので、型の同一性には関わらない。実装（`fn`）は自分で名前を付け、位置で対応させる。TypeScript の関数型で引数名が必須だが代入互換性に影響しないのと同じである。名前を省いて型だけ書くことはできない。`(String)` と書けたなら、それが「String という型」なのか「String という名前」なのか読めなくなる。

引数ゼロは `()` と書く（`現在時刻` のような時刻取得）。「引数が無い」ことと「`Unit` を一つ取る」ことは別で、`Unit` は表層に出さない（7.3）。
...
どのアームが「本線から外れる失敗」かは、この behavior 単体では決まらない。`>>` で合成したとき、**すぐ次の段**が消費しないアームが本線を離れる＝それが本線から外れる（14章）。`承認権限なし` を次の段が入力に取ればそのアームは処理され、取らなければ本線を離れてそのまま出力へ運ばれる。同じ値がある合成では外れ、別の合成では本線に乗る。

入力が直和のこともある（`(提出済み OR 事前承認済み) AND ...`）。パラメータ型に直和を書き、実装で `match` する。名前付きの直和にしておくと読みやすい。
...
dataを生成する behavior は `constructs` を宣言する。これは「生成してよい」という権限の明示であり、依存とは別物なので、実装から推論せず必ず書く。
...
**実装のどこで生成しても宣言が要る。** `require ... else` の離脱値（16.4）も例外ではない。`else 承認権限なし` は単位dataを一つ作っているので、`constructs 承認権限なし` が要る。失敗を表すアームだからといって権限が免除されることはない——2.1 が閉じているのは生成であって、成功か失敗かは関係しない。

#### なぜ実装ではなく仕様に書くか

`constructs` は実装の権限である。それを実装（`fn`）ではなく `behavior` に書くのは、次の三つによる。

2.1 がそう定めている。値を生成できるのは「T の Decoder」「`constructs T` を宣言した **behavior**」「コンパイラ生成コード」であって、権限を持つのは behavior である。

二つの実装経路が同じ宣言から権限を得る。Souther の `fn` はそこに挙がった型を鋳造してよく、Java 実装は同じ宣言から `protected` ファクトリを受け取る（13.3）。`fn` 側に書くと、Java 実装には書く場所が無いので非対称になる。

そして `data` に先例がある。`data 金額 = { value: Int  invariant value >= 0 }` は仕様DSLの `data 金額 = 整数 // 0以上` に `invariant` を足した形である。仕様DSLがコメントに預けたものを実行可能にした分が宣言に乗る、というのが Souther の作りで（1.1）、`constructs` はその表の2行目そのものである。`invariant` が `data` に乗るなら、`constructs` は `behavior` に乗る。

助けの `fn`（対応する behavior を持たないもの）は data を構築できない。Souther の data はすべてドメインのデータなので、**ドメインの値を鋳造することはドメインの行いであり、それには behavior の宣言が要る**。助けの `fn` は述語・算術・文字列処理のように、既にある値を計算するものに留まる。ブロックは別で、それを囲む behavior の権限の下で構築できる（12.5）。

依存（`requires`）は宣言しない。コンパイラが実装から要求集合を推論して型に載せる（13.5、14.3）。生成権限（`constructs`）だけを書き、依存は書かない、が原則である。
...
fn 明細を検証する (xs) = map(xs, x => 1件を検証する(x))
```

ブロックは**無名の `fn`** である。名前を付けないだけで、`fn`（13章）と同じものと考えてよい。名前を付けたものは名前で渡せる。

```text
fn 妥当か (r: 事前承認理由) = ...

all(理由リスト, r => 妥当か(r))     // 無名のブロックで包む
all(理由リスト, 妥当か)             // 名前で直接渡す。同じこと
```

ブロックと `fn` は引数として渡せ、`fn` は引数に取ることもできる（13.6）。**data のフィールドに置くことだけができない。** 関数は外部表現に写せないので、置けたとしても decoder / encoder の導出（10章・11章）の対象外という例外を作ることになる。これは副作用とは無関係の、表現の制約である。仕様DSL自身も、差し替えたい規則を関数ではなく data として持っている（`data フィールド定義 = ... AND バリデーションルール?`）。

無名のブロックは、渡し先へ展開される。逃げ出せないので閉包を作る必要がない。これは実装の都合であって、意味論上の制限ではない。

#### なぜ要求集合が関数の型に出ないか

ここは効果型の言語と分かれるところなので、理由を書いておく。他所の規則をそのまま持ち込むと、要らない制限を作ることになる。

`map(xs, f)` で `f` が外界に触れるなら、`f` の要求を誰が知る必要があるのか。Koka や Unison では `map` である。あちらの副作用は**呼んだ瞬間に発生し、外側のハンドラが動的に捕まえる**ので、`map` が `f` を呼べば `map` が `f` の副作用を起こす。その事実は `map` の呼び出し元へ伝わるから、型に書くしかない（`map : (list<a>, a -> µ b) -> µ list<b>`）。検査例外に近い。`f` が投げるなら呼んだ側も投げる、が連鎖するので、高階関数は引数の効果を型変数で受けるしかなくなる。

Souther の要求集合はそうではない。**コンストラクタ引数である。**

```java
final class 記録する {
    private final Behavior 現在時刻;      // 注入フィールド
    記録する(Behavior 現在時刻) { ... }
}
```

`new 記録する(clock)` を作った時点で依存は中に入り、出来上がったものはただの関数になる。動的スコープが無いので、逃げる先の「外」も無い。だから渡された側は、それが何を要していたか知らなくてよい。要求は**その名前を書いた側**が払い、それは静的に分かる（13.6）。

Effekt がこれを言い当てている。effect 型を「その計算が文脈から要求する capability の集合」と定義し（Souther の要求集合と同じ読み方である）、意味論を capability を追加の引数として渡す calculus への変換で与えたうえで、**その体系の型システムは effect 型を持たない**と述べている（Brachthäuser, Schuster, Ostermann, *Effects as Capabilities*, OOPSLA 2020）。引数で渡してしまえば追跡するものが無くなる。Effekt が表層で effect 型を要るのは受け渡しを推論して隠しているからで、Souther は Java 境界で `bind` に人間が書く。

したがって Koka が union による設計を棄却した理由——`µ1 ∪ µ2 ∼ µ3 ∪ µ4` が一意に解けない（Leijen, *Koka: Programming with Row Polymorphic Effect Types*, MSFP 2014）——は Souther には及ばない。あれは要求集合に**変数**が入る場合の話で、Souther では入らない。Effekt が block を second-class にしている理由（capability がハンドラの動的な範囲に紐づき、逃げると宙に浮く）も同様に当てはまらない。

---

## 13. fn — 実装

`fn` は behavior の実装である。behavior が仕様（12章）で、`fn` がそれを満たすもの、という分業になる。

### 13.1 宣言と束縛

`fn` は `fn 名前 (引数) = 本体` と書く。同名の behavior があれば、それを実装する。
...
型は behavior から取る。`fn` 側は引数に名前を付けるだけで、位置で対応する。TypeScript の `type F = ...` と `const f: F = (a, b) => ...` の関係と同じで、behavior 側の名前は読み手のため、`fn` 側の名前が束縛する。`constructs` は behavior にあるので `fn` には書かない（12.3）。

対応する behavior が無い `fn` は**助け**である。ドメインの振る舞いではないので behavior の一覧を汚さず、`>>` の段にも置けず、`exposing` にも出ない。型は自分で書く。

```text
fn 正の数 (v: Int) = v >= 0
fn 適用する (xs: List<Int>, p: (Int) -> Bool) = all(xs, p)
```

戻り値の型は推論する。宣言したいならそれは仕様なので、behavior を書く。

本体は式である。`{ ... }` はブロック式（16.5）で、`let` や `require` を置きたいときに書く。`fn 正の数 (v: Int) = v >= 0` と `fn 正の数 (v: Int) = { v >= 0 }` は同じものである。

`=` が名前付きの定義を、`=>` が無名のブロック（12.5）を導く。SML の `fun f x = e` と `fn x => e` の分業と同じで、`->` は型の矢印に残る。名前付きの定義にラムダを束縛する形（`fn f = (x) => ...`）は採らない。ML 族はそれを糖衣の側へ書き直す慣習を持っており（HLint の "Redundant lambda"、OCaml マニュアルの `let f x = e` 優先）、その形は書き直しを促す合図として読まれる。

#### 規則

- 同名の behavior がある `fn` は、引数の型を書かない。型は behavior から取る。数が合わなければコンパイルエラーとする。
- 同名の behavior が無い `fn`（助け）は、引数の型を必ず書く。
- 一つの behavior に `fn` を二つ書くことはできない。
- 助けの `fn` は data を構築できない（2.1・12.3）。`>>` の段にも置けず、`exposing` にも書けない。仕様DSLに載らないものだからである。
- 関数型（`(A) -> B`）を書けるのは `fn` の引数の位置だけである。data のフィールド（12.5）にも behavior の出力にも書けない。behavior の出力は業務上の結果の直和であって、関数は業務上の結果ではない。
- MVPでは `fn` は関数を返せない（25.2）。戻り型は推論するので型を書かずに返せてしまうが、返すと閉包を作る必要が生じる。渡すだけなら渡し先で展開できる（12.5）ので、そこは要らない。要求集合の観点では返しても壊れない——閉じたものを返すだけである——ので、これは実装上の線引きであって意味論上の制限ではない。

### 13.2 実装を書かない behavior — 外界依存

`fn` を持たない behavior は、実装を Java から注入する。仕様DSLの `// 依存:` と `// 副作用:` に対応する。
...
// 仕様DSL: // 依存: 会員リポジトリ
behavior findMember = (id: 会員ID) -> 会員 | 会員なし | 保存データ不正 | DB不通
```

キーワードは要らない。**実装が無いことが、外から与えるという意味である。** 仕様DSLにも `required` に当たる語は無く、依存かどうかはコメントで注記されるだけなので、この形なら DSL の行がそのまま残る。
...
実装を書き忘れると、エラーにならずその behavior が注入対象になる。ただし要求集合は推論されて `bind` の引数に現れる（19.5）ので、Java 側の配線がコンパイルエラーになる。`.sou` の中では捕まらない。

### 13.3 Java基底クラス生成

`fn` を持たない behavior は**抽象基底クラス**として生成する。`Behavior` を継承し、宣言した単位data の出力アームごとに、そのアームを構築する `protected` ファクトリを持つ。
...
実装を書かない behavior を参照するものは、その要求を引き継ぐ。要求集合は宣言せず、コンパイラが推論する。

```text
behavior loadAndConvert = findMember >> toView
```

これは `findMember` の実装を要求する。複数を合成した場合、要求集合は和集合になる。

要求は**名前を書いた側へ外向きに積み上がる**。`fn` が実装を書かない behavior を呼べば、その `fn` の要求集合に入り、その `fn` を名前で使ったものへ積み上がる。どの段階でも呼び出しグラフは静的なので、要求集合は具体的な集合として決まり、型に書く必要が生じない（29章）。

これは `fn` を高階にしても変わらない。

```text
fn 適用する (xs: List<会員ID>, p: (会員ID) -> 会員表示) = map(xs, p)
```

`適用する` の要求集合は空である。`p` が何を要求していても関係しない——**`p` は閉じた状態で渡ってくる**からである。要求はコンストラクタ引数として注入され（19.5）、`new 記録する(this.現在時刻)` を作った時点で中に入る。渡された側は、それが何を要していたか知る必要がない。

これが Souther の要求集合と、Koka や Unison の effect row との違いである。あちらの副作用は呼んだ瞬間に発生して外側のハンドラが動的に捕まえるので、`map` が `f` を呼べば `map` が `f` の副作用を起こし、その事実を型に書くしかない（`map : (list<a>, a -> µ b) -> µ list<b>`）。Souther の要求は動的スコープを持たず、注入された時点で閉じるので、伝播する先が無い。Effekt が意味論を capability を引数として渡す calculus への変換で与え、その体系の型システムが effect 型を持たないのと同じ理屈である。`bind` はその引数を部分適用する操作にあたる。
...
段は**単一入力**（1引数の behavior）である。`>>` は「1つの値を次段へ渡す」パイプラインの逐次接続なので、複数引数の behavior（例: `事前承認する(申請, 承認者ID)`）は段には置かず、実装の中でインラインに呼ぶか `match` で受ける。

段になるのは behavior であって `fn` ではない。`>>` は仕様の側の合成で、仕様DSLもそう書く（`behavior 却下して差し戻す = 却下する >> 差し戻す`）。助けの `fn` は仕様DSLに現れないので、段にも置けない。
...
パイプラインは**本線**を運ぶ。段に渡すのは本線だけで、段が受け取らなかったアームは本線を離れ、そのパイプラインの残りの段には**二度と渡らない**。離れたアームは出力に積まれる。

前段の出力を直和 `O_f`、後段の入力型を `I_g` とする。`O_f` のうち `I_g` が受け取れるアームを後段へ流し、残りのアーム `R` は本線を離れる。
...
`h` は `C` を受け取れるが、`C` は `g` の時点で本線を離れているので `h` には渡らない。`h` に渡るのは `g` が出した `D` だけである。**これが Railway** であり、離れたアームを後の段に見せると Railway ではなくなる。「失敗」を特別扱いしているのではない——消費されなかったアームが本線を離れるだけで、どのアームが離れるかは合成の相手が決める（2.6）。

出力の直和は open error row として扱い、`(C | R) | S` を平坦化し、同名アームは畳む。`O_f` のどのアームも `I_g` に合わなければ合成できない（22.14）。

**本線を離れたことは、そのパイプラインの中でしか覚えていない。** 途中に名前を付けると、そこで忘れる。

```text
behavior fg = f >> g          // fg : A -> D | C
behavior x  = fg >> h         // h は D も C も受け取る。x : A -> E
```

`fg` の型は「`D` か `C` を返す」としか言っておらず、「`C` は本線を離れたやつだ」という印はどこにも無い。値が印を持たないのは 2.6 のとおりで、`C` がエラーかどうかは合成の局面で決まる。`fg` は正直に「`D` か `C` を返す」と宣言し、`h` は正直に「`C` か `D` を受ける」と宣言しているので、`h` が `C` を受け取るのは正しい。`fg` の中で `C` が離れたことは `fg` の内部事情である。

したがって `f >> g >> h` と、途中に名前を付けた `fg >> h` は**別物になりうる**。`>>` は関数合成ではなく、本線を段に通す演算子である。名前を付けることは入力型を宣言する行為で、そこが抽象の境界になる。

`>>` は左結合とする（`f >> g >> h` は `(f >> g) >> h` と同じ平らな1本のパイプラインである）。
...
再代入は禁止する。`let` はブロック式（16.5）の中に置く。`let x = v` に続く残りがその束縛の届く範囲で、これは脱糖の時点で入れ子になる。
...
`require` は、出力に離脱アームを持つ behavior の実装（`fn`）の中で使う。条件が偽なら `else` の値を返してその場で終える。`else` の値は出力の直和のいずれかのアーム（ただの data）である。実装は、返したいアームの値を最後の式に置くだけでよい。特別な包み込みは無い。
...
置くものが結果の式だけなら、ブロックは要らない。`fn 正の数 (v: Int) = v >= 0` と `fn 正の数 (v: Int) = { v >= 0 }` は同じものである。

ブロックは式なので、式が置ける場所ならどこにでも書ける（`if c then { let x = f(a)  g(x) } else h`）。中身は 16.1 と 16.4 の規則どおり脱糖され、AST には束縛と `if` として現れる。裸の `{` はブロック式で確定する——レコードリテラルは型名が前に付く（12.4）ので、取り違えは起きない。
...
外界依存は実装を書かない behavior として宣言し、実装は Java 側から注入する（13.2）。`fn` はそれを呼べるので、`fn` の結果が外界に左右されることはある。ただし触れ方は必ず behavior の呼び出しを経由し、その事実は要求集合に現れる（13.6）。これが SMDD の DDDトリレンマの結論——純粋性はレイヤーの分離ではなく、どの関数が副作用を持つかを明示すること——にあたる（13.5）。

**「要求集合が空である」ことは、この意味の純粋性より強い。** この章が言うのは「直接は触れない」ことで、すべての `fn` が満たす。要求集合が空とは、実装を書かない behavior を推移的に一度も呼ばないことで、一部の `fn` だけが満たす。invariant から呼べるのは後者である（9.3）。二つを混同しないよう、以降は後者を「要求集合が空」と呼び、「純粋」とは呼ばない。
...
ブロックの中から、実装を書かない behavior を呼べば、その要求は呼び出し元の要求集合へ積み上がる。要求集合は書かない（29章）。
...
DecoderDef  EncoderDef  BehaviorDef  FnDef  Import
Construct  FieldInit  Expression  Case  RawExpr  DecRef  Block
```

`BehaviorDef` は仕様だけを持ち、本体を持たない（12章）。同名の `FnDef` があればそれが実装で、無ければ Java 注入の対象になる（13.2）。`SumData` のアームは名前付きdataへの参照だけを持つ（8.3）。`include` は取り込むフィールドと invariant を平らに引き継ぐ。`DecoderDef` / `EncoderDef` は導出（10章・11章）が data の形状から充填する。`require` と `let` は脱糖済みなので AST には現れず、`if` とブロックの束縛として現れる（16.4・16.5）。
...
出力型は単一型または直和。要求集合（必要な、実装を書かない behavior）は実装から推論する（13.6・14.3）。バックエンドはこの情報を使い、AST から直接バイトコードを生成する。
...
Declare a behavior without an `fn` and provide its implementation from Java.
```
...
fn 事前承認要否を判定する (予定費用, 申請者, 費用負担区分) = { ... }

fn 出張申請を提出する (申請) = { ... }

// 事前条件: 承認者IDが申請者の上長IDと一致すること
// 現在時刻() を呼ぶので、要求集合 {現在時刻} はコンパイラが推論する。書かない
fn 事前承認する (申請, 承認者ID) = {
    require 承認者ID == 申請.申請者.上長ID
        else 承認権限なし
...
`.sou`読み込み、Lexer、Parser、AST、モジュール、import、`data`（直積・直和・単位・アーム参照・`include`）、`?`、`invariant`、形状からの `decoder` / `encoder` 導出（newtype・JSONキー＝フィールド名・判別子 `"type"`／アーム名タグ）、`behavior`（仕様のみ。引数ゼロの `()` を含む）、`fn`（同名の behavior への束縛と、助けの fn）、実装を書かない behavior の Java 注入、`constructs`、要求集合の推論、レコードリテラルと spread と `with`、ブロックと高階の `fn`（12.5・13.6）、印の無い直和出力（失敗もアーム）、`>>`（型ルーティング合成）、`match`、`let`、`if`、`require`（`if` への脱糖。16.4）、`Option`、`NonEmptyList`、null禁止、例外禁止、網羅性検査、生成権限検査、invariant違反アーム検査、behavior合成型検査、助けの fn が data を構築しないことの検査、invariant から呼ぶ fn の純粋性検査（9.3）、ClassFile バイトコード生成（Java 21 のクラスファイルバージョンで出力。19.1）、Java基底クラス生成、Raoh の decoder/encoder としての生成（入力源ごと＝素の値/Map・JSON(`JsonNode`)・jOOQ `Record`、集積・判別は Raoh の combinator、souther-runtime は Raoh 非依存）、Unicode識別子、単体テスト。

### 25.2 後回し

Javaソース生成backend（人間可読な生成コード）、増分コンパイル、IDEプラグイン、LSP、ソースマップ、関数を data のフィールドに格納すること（外部表現に写せないため。12.5）、`fn` が関数を返すこと（閉包の生成が要る。13.1）、高度な型推論、静的不変条件証明、structural intersection types、手書き decoder / encoder 構文（別キー名・正規化・業務固有の判別子は境界のカスタムコーデックで扱う）、入力源を使われ方で絞る生成の最適化（現状は data の形状が対応する源ぶんを生成する。10.6）、JSON temporal / jOOQ 入れ子の対応（現状はそれぞれ JSON temporal を持つ型 / 入れ子を持つ型では該当源の decoder を生成しない）、JSON Schema生成、Wasm出力、JavaScript出力、非同期の外界依存。
...
data 出張申請（状態の直和、共通項目は include）
  ...
  + behavior 事前承認する（仕様: -> 事前承認済み | 承認権限なし, constructs）
  + fn 事前承認する（実装: require, 現在時刻() 依存の推論）
  + behavior 現在時刻（実装なし → Java 注入）
```
...
fn createMember (email) = Member { email }
```
...
失敗を分ける第4の枠は無い。失敗は Output の一部のアームで、本線から外れるかどうかは合成で決まる。`>>` は、出力アームの振り分け（後段が消費するアームを流し、残りを伝播）と要求集合の合成を同時に行う。
