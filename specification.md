...
ブロックは値ではない。behavior の戻り値にすること、data のフィールドに格納すること、`let` で束縛することを禁じる。許すのは引数として渡すことだけである。渡す先は標準ライブラリ（18章）でも、ブロックを引数に取るユーザー定義の behavior でもよい。ブロックの型（`(A) -> B`）は引数の位置にしか書けない。
