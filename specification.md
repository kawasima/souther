...
`Never` と `Unit` は**表層で書ける型名ではなく、読み方の概念**である。`Never` は起こり得ないアームを表し、単一アームの出力（`-> A`）は失敗row が空、と読める。`Unit` は仕様DSLの `単位型` に対応するが、フィールドを書く手段としては提供せず、常に「フィールドを持たない data」（8.3）で表す。`NonEmptyList<T>` は Decoder のエラー集積のためにランタイムが持つが、ユーザーがフィールド型として書くものではない。
...
```text
Decoder<T> = Raw -> T | 復号失敗          // 復号失敗 は NonEmptyList<DecodeError> を運ぶ
```
...
- Decoder: 違反は復号失敗（`NonEmptyList<DecodeError>` を運ぶアーム）に入る。Decoderの出力が常に `T | 復号失敗` なので必ず満たす（10章）。
...
- 構築時に invariant を検査し、違反は `DecodeError` になる。Decoder の出力が常に `T | 復号失敗` なので、違反の行き先は常にある（9.4）。
...
Decoder の出力 `T | 復号失敗` の失敗アームが `復号失敗` である。集積した独立エラーをまとめて運ぶ（15章）。

```text
data 復号失敗 {
    errors: NonEmptyList<DecodeError>
}
```

### 10.6 生成方式とRaoh

導出した Decoder は、独立した Decoder IR や「Raoh backend」という中間層を経ず、ClassFile backend が**直接バイトコードとして生成する**。生成された `decode` 本体は、ランタイムの `Decoders` facade（18.6）の静的メソッドを呼ぶ。

```text
Source -> AST -> (導出で decoder/encoder を AST に充填) -> 型検査 -> ClassFile backend -> .class
```

プリミティブ解析の一部（`text` / `int`）は `Decoders` facade の内部で Raoh に委譲するが、`decimal` / `bool` / `date` / `dateTime` は facade が直接扱う。Raoh固有型は facade の内側に閉じ、言語の公開APIにも生成バイトコードにも露出しない。将来 Decoder を最適化する余地として IR を挟むことはできるが、MVP では持たない。判別振り分け（10.3）は生成された sum の `decode` が直接行う。
...
段は**単一入力**（1引数の behavior、または `Type.decoder` / `Type.encoder`）である。`>>` は「1つの値を次段へ渡す」パイプラインの逐次接続なので、複数引数の behavior（例: `事前承認する(申請, 承認者ID)`）は段には置かず、呼び出し（`match ...(x, y) { ... }`）や本体内のインライン呼び出しで使う。`Type.decoder` は `Raw -> T | 復号失敗`、`Type.encoder` は `T -> Raw` の段として振る舞い、境界の Raw を段の入出力に持ち込める。
...
## 15. Decoderのエラー集積とbehaviorのアーム伝播
...
集積した失敗は復号失敗（`NonEmptyList<DecodeError>` を運ぶアーム）にまとまる。Decoder の出力は `T | 復号失敗` なので、通常の behavior と `>>` でつながる。復号失敗のアームを後段が受け取らなければ、そのまま素通りして末尾へ運ばれる（14.2）。

```text
Decoder内部      独立エラーを集積（Applicative, NonEmptyList<DecodeError>）
behavior >>      消費されないアームを素通りで伝播
```

集積（applicative）とアーム伝播は、同じ「型付き結果の直和」に対する2つの合成様式である。集積が成り立つのは、Decoder の各フィールド取得が互いに独立（前の値に依存しない）だからである。
...
`require` は Decoder、または出力に離脱アームを持つ behavior 内で使う。条件が偽なら `else` を返してその場で終える。Decoder内では `else` の識別子が `DecodeError.code` になる。behavior内では `else` の値が出力の直和のいずれかのアーム（ただの data）になる。behavior の本体は、返したいアームの値を最後の式に置くだけでよい。特別な包み込みは無い。
...
### 18.6 Decoders / Encoders facade

Souther の構文には現れない、ランタイムの facade API である。生成された `decode` / `encode` バイトコードがこれを呼び（10.6）、境界のカスタムコーデック（10.4）は Java 側から同じ facade を使う。プリミティブ解析は facade の内側に閉じ、`text` / `int` は Raoh へ委譲、`decimal` / `bool` / `date` / `dateTime` は facade が直接扱う。

```text
field  optionalField  objectField  index  list  variant
text  integer  decimal  bool  date  dateTime
```
...
具体的な Decoder / Encoder 実装は ClassFile backend が直接バイトコードとして生成し、ランタイムの `Decoders` / `Encoders` facade を呼ぶ（10.6）。
...
`SumData` のアームは名前付きdataへの参照、またはインライン定義を持つ。`include` は取り込むフィールドと invariant を平らに引き継ぐ。`DecoderDef` / `EncoderDef` は導出（10章・11章）が data の形状から充填する。
    ...
    public FindMemberResult apply(MemberId id) {   // sealed: Member | FindMemberError の直和
        try (var connection = dataSource.getConnection();
            ...
            try (var resultSet = statement.executeQuery()) {
                ...
                // decode の出力は Member | 復号失敗。復号失敗は保存データ不正のアームへ写す
                return switch (Member.decoder().decode(raw)) {
                    case Member m  -> m;
                    case 復号失敗 f -> new FindMemberError.InvalidStoredData(f);
                };
            }
...
11. ランタイム `Decoders` facade（プリミティブ解析、`text`/`int` は Raoh へ委譲）
...
```text
data 金額 / 従業員ID  + invariant + 導出decoder/encoder + ランタイムfacade + ClassFile生成
```
...
- Raoh依存をランタイムの `Decoders` facade へ隔離する（コンパイラ本体・生成バイトコードに露出させない）
