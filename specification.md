...
`require` は Decoder、または出力に離脱アームを持つ behavior 内で使う。条件が偽なら `else` を返してその場で終える。Decoder内では `else` の識別子が Raoh の `Issue` の code になる。behavior内では `else` の値が出力の直和のいずれかのアーム（ただの data）になる。behavior の本体は、返したいアームの値を最後の式に置くだけでよい。特別な包み込みは無い。
